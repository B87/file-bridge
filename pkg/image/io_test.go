...
func TestOpenSave(t *testing.T) {
	...
	for _, ext := range []string{"jpg", "jpeg", "png", "gif", "bmp", "tif", "tiff"} {
		...
		for _, opts := range options {
			...
			img2, err := Open(filesys.NewURI(filesys.LocalScheme, filename))
			if err != nil {
	...
	prevFS := fileSystem
	fileSystem = filesys.NoopFS{}
	defer func() { fileSystem = prevFS }()

	err = Save(imgWithAlpha, "test.jpg")
	if err != filesys.ErrFileCreate {
		t.Fatalf("got error %v want ErrFileCreate", err)
	}

	err = Save(imgWithAlpha, "badFile.jpg")
	if err != filesys.ErrFileClose {
		t.Fatalf("got error %v want ErrFileClose", err)
	}

	_, err = Open(filesys.NewURI(filesys.LocalScheme, "test.jpg"))
	if err != filesys.ErrFileOpen {
		t.Fatalf("got error %v want ErrFileOpen", err)
	}
}
...
func TestAutoOrientation(t *testing.T) {
	...
	for _, tc := range testCases {
		img, err := Open(filesys.NewURI(filesys.LocalScheme, tc.path), AutoOrientation(true))
		if err != nil {
			t.Fatal(err)
		}
